...
//        }
    ...
    public Page<ReadUsersFeedsResponseDto> readUserFeed(
            Long userId,
            PrincipalRequestDto principalRequestDto,
            Pageable pageable
    ){
        Optional<Users> user = usersRepository.findById(userId);
        if (user.isEmpty()) {
        ...
        Map<Long, Boolean> liked = likeRepository.isLikedByFeedIdsANDUserId(feedIdsList, principalRequestDto.getUserId()).stream()
                                        .collect(Collectors.toMap(row->(Long) row[0], row ->(Boolean) row[1]));


        Map<Long, List<String>> feedImgs = feedImgRepository.findFeedImgByFeedId(feedIdsList).stream()
