...
//        System.out.println("[JWT] Authorization header = " + header);
        ...
        if ("POST".equalsIgnoreCase(httpServletRequest.getMethod())
                && ("/api/auth/signin".equals(httpServletRequest.getRequestURI()))
            ){
        ...
        if (header != null && header.startsWith("Bearer ")) {
            ...
            if (JwtUtil.validateToken(token)) {
                ...
                boolean isWithdrawEndpoint = "DELETE".equalsIgnoreCase(httpServletRequest.getMethod())
                                && "/api/myinfo/modify/delete".equals(httpServletRequest.getRequestURI());
                if (isWithdrawEndpoint) {
                    usersRepository.findByEmail(email).ifPresent(u -> {
                        if (Boolean.TRUE.equals(u.getDeleted())) {
                            try {
                                httpServletResponse.sendError(HttpServletResponse.SC_CONFLICT, "이미 탈퇴한 사용자입니다.");
                            } catch (IOException ignored) {}
                        }
                    });
                    if (httpServletResponse.isCommitted()) return; // 이미 409 보냈으면 종료
                }
